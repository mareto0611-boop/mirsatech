...
async function openIngresoDetalle(iid) {
  ...
  try {
      ...
      <div class="card">
        <div class="flex" style="justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
          ...
          <div class="flex" style="gap:8px;flex-wrap:wrap;justify-content:flex-end;">
            <button class="btn-ghost" id="btnRefreshDet" type="button">Recargar</button>
            <button class="btn" id="btnNewEvo" type="button">+ Evolución</button>
            <button class="btn" id="btnNewOrd" type="button">+ Orden</button>
            <button class="btn-ghost" id="btnEpicrisis" type="button">${ing.estado === "egresado" ? "Ver/Editar Epicrisis" : "Egresar (Epicrisis)"}</button>
          </div>
        </div>
    ...
    detailEl.querySelector("#btnRefreshDet").addEventListener("click", () => openIngresoDetalle(iid));

    btnNewEvo.disabled = !canMed;
    ...
    const setActive = (id) => {
      ["tabAdm","tabOrd","tabEvo","tabEpi"].forEach(x => detailEl.querySelector("#"+x).classList.remove("btn"));
      detailEl.querySelector("#"+id).classList.add("btn");
    };

    detailEl.querySelector("#tabAdm").addEventListener("click", () => { setActive("tabAdm"); renderTabAdmision(body, iid, adm, ing, canAdm, () => openIngresoDetalle(iid)); });
    detailEl.querySelector("#tabOrd").addEventListener("click", () => { setActive("tabOrd"); renderTabOrdenes(body, iid, ord, canMed, () => openIngresoDetalle(iid)); });
    detailEl.querySelector("#tabEvo").addEventListener("click", () => { setActive("tabEvo"); renderTabEvoluciones(body, iid, evol, canMed, ing, () => openIngresoDetalle(iid)); });
    detailEl.querySelector("#tabEpi").addEventListener("click", () => { setActive("tabEpi"); renderTabEpicrisis(body, epi); });
...
function renderTabAdmision(body, iid, adm, ing, canEdit, onSaved) {
  ...
  body.querySelector("#btnSaveAdm").addEventListener("click", async () => {
